...
# El algoritmo de Euclides extendido se puede utilizar para verificar si `e` es coprimo con `φ(n)`
...
def generar_claves_rsa(bits=1024):
    ...
    return {
        'public_key': (e, n),
        'private_key': (d, n),
        'values': (p, q, dp, dq, q_inv) # Valores que utilizará la función CRT
    }